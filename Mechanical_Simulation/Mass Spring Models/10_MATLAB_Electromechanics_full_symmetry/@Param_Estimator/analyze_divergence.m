...
%ixs=abs(sqrt(sum(obj.plate.v.^2)))>0; %Sometimes squaring the value exceeds the maximum value allowed by matlab, using sum(abs(v)) is more robust and conservative

ixs=sum(abs(obj.plate.v))>0;
if any(ixs) % If all particles are still, there is no risk of this kind of oscillation
    v=sum(abs(obj.plate.v(:,ixs)));
    c=obj.plate.damp_factor;
    dm=obj.plate.dm;
    dts=2*(dm./v)/c;
    reco_dt1=min(dts);
    if reco_dt1<dt
...
dp_def=obj.plate.dp_def;
dp_x=obj.plate.x_rel(:,2:end-1);
dp_v=diff(obj.plate.v,1,2); % Vi-Vii
...
% Check for LTR
y_ltr=obj.plate.local_frame_y_ltr;
dp_v=-diff(obj.plate.v,1,2); % -(Vi-Vii) % I really need to check the signs here, it makes the world of a difference.
y_def=obj.plate.local_def_y_ltr;
...
%Check for RTL
y_rtl=obj.plate.local_frame_y_rtl;
dp_v=diff(obj.plate.v,1,2); % (Vi-Vii) % I really need to check the signs here, it makes the world of a difference.
y_def=obj.plate.local_def_y_rtl;
